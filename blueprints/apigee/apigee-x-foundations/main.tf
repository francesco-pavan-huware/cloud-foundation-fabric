...
locals {
  prefix = var.prefix == null ? "" : "${trim(var.prefix)}-"
}
...
module "project" {
  ...
  org_policies_data_path  = var.project_config.org_policies_data_path
  ...
  service_perimeter_bridges  = var.project_config.service_perimeter_bridges
  service_perimeter_standard = var.project_config.service_perimeter_standard
...
module "vpc" {
  count      = try(var.shared_vpc_config.use_for_apigee, false) && length(local.instances) > 0 ? 0 : 1
  source     = "../../../modules/net-vpc"
  project_id = module.project.project_id
  name       = "${local.prefix}vpc"
  psa_config = {
    ranges = merge(flatten([for k, v in var.apigee_config.instances : merge(
      v.runtime_ip_cidr_range == null ? {} : { "apigee-runtime-${k}" = v.runtime_ip_cidr_range },
      v.troubleshooting_ip_cidr_range == null ? {} : { "apigee-troubleshooting-${k}" = v.troubleshooting_ip_cidr_range }
    )])...)
    export_routes  = true
    import_routes  = false
    peered_domains = local.peered_domains
  }
  subnets = [for k, v in var.apigee_config.instances :
    {
      name          = "subnet-${k}"
      region        = k
      ip_cidr_range = v.subnet.ip_cidr_range
      description   = "Subnet in ${k} region"
    }
  if v.subnet.ip_cidr_range != null]
  subnets_proxy_only = [for k, v in var.apigee_config.instances :
    {
      name          = "subnet-proxy-only-${k}"
      region        = k
      ip_cidr_range = v.subnet_proxy_only_ip_cidr_range
      description   = "Proxy-only subnet in ${k} region"
    }
  if v.subnet_proxy_only_ip_cidr_range != null]
  subnets_psc = [for k, v in var.apigee_config.instances :
    {
      name          = "subnet-psc-${k}"
      region        = k
      ip_cidr_range = v.subnet_psc.ip_cidr_range
      description   = "PSC Subnet in ${k} region"
    }
  if v.subnet_psc.ip_cidr_range != null]
}
